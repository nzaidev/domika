...
export async function renderFlyerImage(
  ...
  lines.push(
    `<text x="60" y="${cursor}" font-size="52" font-weight="700" fill="#ffffff" font-family="Helvetica, Arial, sans-serif">${escapeXml(truncate(data.title, 34))}</text>`,
  );
  cursor += 52;

  if (data.location) {
    lines.push(
      `<text x="60" y="${cursor}" font-size="30" fill="rgba(255,255,255,0.85)" font-family="Helvetica, Arial, sans-serif">${escapeXml(truncate(data.location, 52))}</text>`,
    );
    cursor += 62;
  } else {
    cursor += 20;
  ...
  if (sections.includes("price")) {
    lines.push(
      `<text x="60" y="${cursor + 30}" font-size="72" font-weight="700" fill="#ffffff" font-family="Helvetica, Arial, sans-serif">${escapeXml(data.priceLabel)}</text>`,
    );
    lines.push(
      `<text x="${WIDTH - 60}" y="${cursor + 30}" text-anchor="end" font-size="30" fill="rgba(255,255,255,0.85)" font-family="Helvetica, Arial, sans-serif">${escapeXml(data.operationLabel)}</text>`,
    );
    cursor += 110;
  }
  ...
  if (sections.includes("specs") && specLine) {
    lines.push(
      `<text x="60" y="${cursor}" font-size="28" fill="rgba(255,255,255,0.92)" font-family="Helvetica, Arial, sans-serif">${escapeXml(truncate(specLine, 70))}</text>`,
    );
    cursor += 52;
  }
  ...
  if (amenityLine) {
    lines.push(
      `<text x="60" y="${cursor}" font-size="26" fill="rgba(255,255,255,0.75)" font-family="Helvetica, Arial, sans-serif">${escapeXml(truncate(amenityLine, 74))}</text>`,
    );
  }
...
<svg width="${WIDTH}" height="${HEIGHT}" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="60" y="${HEIGHT - 38}" font-size="28" font-weight="700" fill="#ffffff" font-family="Helvetica, Arial, sans-serif">${escapeXml(truncate(footer, 68))}</text>
</svg>`;
