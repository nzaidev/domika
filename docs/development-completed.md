...
- **Token-at-rest encryption**: Meta/WhatsApp access tokens are encrypted with AES-256-GCM (`src/lib/crypto/secret-box.ts`, keyed by `ENCRYPTION_KEY`) before storage and decrypted only server-side at use. Set `ENCRYPTION_KEY` (`openssl rand -base64 32`) in prod; without it, tokens store as plaintext (dev fallback with a warning).
- **Adversarial security review (2026-07)** — an agent audited tenant isolation, PII paths, API auth, webhook sigs, upload validation, secrets, and injection. Confirmed clean: owner-PII cross-org defenses, `.or()` search (blocklist + AND-scoped to org), file-upload validation, `/api/documents` + `/api/properties/[id]/media` authz, webhook HMAC (constant-time, raw-body). **Findings fixed:** WhatsApp inbound media (customer PII) was going to the *public* bucket → now the private `documents` bucket served via authed `/api/documents` (and the middleware matcher now always runs on `/api/*` so image-extension document URLs still hit Clerk); dead cross-tenant listing-distribution write functions removed; `createTask` now verifies linked lead/property belong to the caller's org; Meta Graph token moved from URL query to `Authorization` header; cron secret compared with `timingSafeEqual`. **Residual (documented, low):** published *and draft* property photos share the public `domika-fotos` bucket at unguessable UUID paths — acceptable for marketing images, but a bucket split (public-listing vs private) is the clean pre-scale hardening.
